...
if isempty(SWITCH) | done<0.03
    ...
    prevDone = done;
    %aString = repmat(spaceChar, 1, nCols);
end
...
% aString(end) = [];
% if cyclesLeft>(txSpSizeMax(2) + nCols)
%     aString =[char(randi(charRange,1))  aString];
% else
%     aString =[spaceChar  aString];
% end
% nLine = ceil(done*nCols);
% 
% prTx = [' %' sprintf('%3d',round(done*100))];
% aString(end-4:end) = prTx;
...
% aString(:) = spaceChar;
% if mod(round(done*50),1.2) == 0
%     indLoc = randi(nCols-4,1);
%   
% end
disp(dispMain)
disp(aString)
